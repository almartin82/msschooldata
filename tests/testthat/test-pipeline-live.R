# ==============================================================================
# LIVE Pipeline Tests for msschooldata
# ==============================================================================
#
...
test_that("Mississippi DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add Mississippi DOE URL and verify HTTP 200")
})
...
test_that("Can download Mississippi enrollment data file", {
  skip_if_offline()
...
test_that("Can parse Mississippi enrollment file", {
  skip_if_offline()
...
test_that("msschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- msschooldata::get_available_years()
  ...
  tryCatch({
    raw <- msschooldata:::get_raw_enr(test_year)
...
test_that("get_available_years returns valid year range", {
  result <- msschooldata::get_available_years()
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- msschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- msschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- msschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- msschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Cache functions exist and work", {
  ...
  tryCatch({
    path <- msschooldata:::get_cache_path(2024, "enrollment")
