...
def get_test_years():
    ...
    if _available_years is None:
        ...
        _available_years = ms.get_available_years()
...
class TestImport:
    ...
    def test_import_functions(self):
        ...
        import pymsschooldata as ms
        assert hasattr(ms, 'fetch_enr')
        assert hasattr(ms, 'fetch_enr_multi')
        assert hasattr(ms, 'tidy_enr')
        assert hasattr(ms, 'get_available_years')
...
class TestFetchEnr:
    ...
    def test_returns_dataframe(self):
        ...
        df = ms.fetch_enr(years['max_year'])
    ...
    def test_total_enrollment_reasonable(self):
        ...
        if 'is_district' in df.columns and 'grade_level' in df.columns:
            ...
            if len(total_df) > 0:
                ...
                # Mississippi should have ~440,000 students
...
class TestFetchEnrMulti:
    ...
    def test_returns_dataframe(self):
        ...
        df = ms.fetch_enr_multi([years['max_year']])
        assert isinstance(df, pd.DataFrame)
    ...
    def test_contains_requested_year(self):
        ...
        df = ms.fetch_enr_multi([test_year])
    ...
    def test_multi_matches_single(self):
        ...
        import pymsschooldata as ms
        years = get_test_years()
        df_single = ms.fetch_enr(years['max_year'])
        df_multi = ms.fetch_enr_multi([years['max_year']])
